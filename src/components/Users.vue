<template>
  <div>
    ...
    <el-dialog title="修改用户" :visible.sync="editDialogVisible" width="50%">
      <el-form @click="editDialogClosed" :model="editForm" label-width="70px" :rules="editFormRules" ref="editFormRef">
    ...
    <!-- 分配角色对话框 -->
    <el-dialog @close="setRoleDialogClosed" title="分配角色" :visible.sync="setRoleFialogVisible" width="50%">
      <div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="setRoleFialogVisible = false">取 消</el-button>
...
export default {
  data() {
    ...
    return {
      ...
      editFormRules: {
        email: [{ require: true, message: '请输入用户邮箱', trigger: 'blur' }, { validator: checkEmail, trigger: 'blur' }],
        mobile: [{ require: true, message: '请输入用户用户的手机', trigger: 'blur' }, { validator: checkMobile, trigger: 'blur' }]
      },
      // 分配角色对话框的显示与隐藏
      setRoleFialogVisible: false,
      // 当前用户信息
      userInfo: [],
      // 所以角色的列表
      rolesList: [],
  ...
  methods: {
    ...
    async showEditDialog(id) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('查询用户数失败')
      }
    ...
    editUser() {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (res.meta.status !== 200) {
          return this.$message.eror('修改用户失败！')
        }
    ...
    async setRole(userInfo) {
      ...
      this.setRoleFialogVisible = true
    },
    async setRoleInfo() {
      ...
      this.getUserList()
      this.setRoleFialogVisible = false
    },
    setRoleDialogClosed() {
      this.selectedRoleId = ''
      this.userInfo = ''
